...
class Options_Framework_Interface {
	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			// Sets pre text description value
			$pre_text_value = '';
			if ( isset( $value['pre'] ) ) {
				$pre_text_value = $value['pre'];
			}
			...
			// Basic text input
			case 'text':
				$output .= '<div class="pre-text">'. $pre_text_value .'</div>'.'<input id="' . esc_attr( $value['id'] ) . '" class="of-input" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" type="text" value="' . esc_attr( $val ) . '"' . $placeholder . ' />';
				break;
